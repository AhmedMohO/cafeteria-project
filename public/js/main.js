...
const productsPaginationContainer = document.querySelector('#products-pagination');
...
function renderProductImage(imageName, altText, className = 'product-thumb', fallbackText = 'No image') {
  ...
  const fallbackMarkup = `<div class="${className} product-image-fallback d-flex align-items-center justify-content-center">${escapeHtml(fallbackText)}</div>`;

  if (!imageUrl) {
    return fallbackMarkup;
  }

  const onErrorHandler = `this.onerror=null;this.outerHTML=decodeURIComponent('${encodeURIComponent(fallbackMarkup)}')`;

  return `<img src="${imageUrl}" alt="${escapeHtml(altText || 'Product image')}" class="${className}" loading="lazy" onerror="${onErrorHandler}">`;
}
...
async function loadProducts(searchTerm = '', page = 1) {
  ...
  try {
    const response = await fetch(`${endpoints.searchProducts}?q=${encodeURIComponent(currentSearchTerm)}&page=${requestedPage}&per_page=${PRODUCTS_PER_PAGE}`, {
      method: 'GET',
      cache: 'no-store',
    });
    ...
    currentProductsPage = Math.max(1, Number.parseInt(pagination.page, 10) || requestedPage);
    currentProductsTotalPages = Math.max(0, Number.parseInt(pagination.total_pages, 10) || 0);
...
function renderProductsPagination() {
  if (!productsPaginationContainer) return;

  if (currentProductsTotalPages <= 1) {
    productsPaginationContainer.innerHTML = '';
    return;
  }

  const visiblePages = buildVisiblePages(currentProductsPage, currentProductsTotalPages);
  const items = [];

  const prevDisabled = currentProductsPage <= 1;
  items.push(`<li class="page-item ${prevDisabled ? 'disabled' : ''}"><a class="page-link" href="#" data-page="${currentProductsPage - 1}">&laquo;</a></li>`);

  let previous = null;
  visiblePages.forEach((pageNum) => {
    if (previous !== null && pageNum - previous > 1) {
      items.push('<li class="page-item disabled"><span class="page-link">...</span></li>');
    }

    items.push(`<li class="page-item ${pageNum === currentProductsPage ? 'active' : ''}"><a class="page-link" href="#" data-page="${pageNum}">${pageNum}</a></li>`);
    previous = pageNum;
  });

  const nextDisabled = currentProductsPage >= currentProductsTotalPages;
  items.push(`<li class="page-item ${nextDisabled ? 'disabled' : ''}"><a class="page-link" href="#" data-page="${currentProductsPage + 1}">&raquo;</a></li>`);

  productsPaginationContainer.innerHTML = `<nav aria-label="Products pagination"><ul class="pagination pagination-sm mb-0">${items.join('')}</ul></nav>`;
}
...
if (productsPaginationContainer) {
  productsPaginationContainer.addEventListener('click', (event) => {
    ...
    const targetPage = Number.parseInt(link.getAttribute('data-page') || '1', 10);
    if (!Number.isInteger(targetPage)) return;
    if (targetPage < 1 || targetPage > currentProductsTotalPages) return;
    if (targetPage === currentProductsPage) return;

    loadProducts(currentSearchTerm, targetPage);
  });
}
