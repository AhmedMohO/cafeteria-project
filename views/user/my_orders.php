...
$appBase = $appBase ?? (defined('APP_BASE_PATH') ? APP_BASE_PATH : '');
...
$buildPageLink = static function (int $targetPage) use ($dateFrom, $dateTo, $app): string {
...
<div class="container-fluid py-4 px-3 px-lg-4">
	...
	<div class="card border-0 shadow-sm mb-4 rounded-4">
		<div class="card-body p-3">
			<form id="my-orders-filter-form" method="GET" action="<?= htmlspecialchars($app('/user/my-orders')) ?>" class="row g-2 align-items-end">
				<div class="col-md-4">
					<label for="date_from" class="form-label small fw-semibold">Date From</label>
					<input id="date_from" name="date_from" type="date" class="form-control" value="<?= htmlspecialchars($dateFrom ?? '') ?>" max="<?= htmlspecialchars($dateTo ?? '') ?>">
				</div>
				<div class="col-md-4">
					<label for="date_to" class="form-label small fw-semibold">Date To</label>
					<input id="date_to" name="date_to" type="date" class="form-control" value="<?= htmlspecialchars($dateTo ?? '') ?>" min="<?= htmlspecialchars($dateFrom ?? '') ?>">
				</div>
				<div class="col-md-2">
					<button class="btn btn-warning w-100 fw-semibold" type="submit"><i class="bi bi-funnel me-1"></i>Filter</button>
				</div>
				<div class="col-md-2">
					<a class="btn btn-outline-secondary w-100" href="<?= htmlspecialchars($app('/user/my-orders')) ?>">Reset</a>
				</div>
			</form>
		</div>
	</div>
